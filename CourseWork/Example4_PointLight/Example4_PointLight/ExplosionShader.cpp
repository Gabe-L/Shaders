...
ExplosionShader::ExplosionShader(ID3D11Device* device, HWND hwnd) : BaseShader(device, hwnd)
{
	initShader(L"explosion_vs.cso", L"explosion_ps.cso");
}
...
void ExplosionShader::initShader(WCHAR* vsFilename, WCHAR* psFilename)
{
...
void ExplosionShader::setShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX &worldMatrix, const XMMATRIX &viewMatrix, const XMMATRIX &projectionMatrix, ID3D11ShaderResourceView* texture, ID3D11ShaderResourceView* texture_two, float time, float explodeOffset)
{
	HRESULT result;
