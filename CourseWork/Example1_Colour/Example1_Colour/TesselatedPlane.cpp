...
TesselatedPlane::TesselatedPlane(ID3D11Device* device, ID3D11DeviceContext* deviceContext, float scale)
{
	initBuffers(device, scale);
}
...
void TesselatedPlane::initBuffers(ID3D11Device * device, float scale)
{
...
void TesselatedPlane::sendData(ID3D11DeviceContext* deviceContext, D3D_PRIMITIVE_TOPOLOGY top)
{
	unsigned int stride;
	unsigned int offset;
