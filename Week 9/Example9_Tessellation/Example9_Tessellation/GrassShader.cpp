...
{
	initShader(L"tessellation_vs.cso", L"grass_gs.cso", L"grass_ps.cso");
}
...
void GrassShader::initShader(WCHAR* vsFilename, WCHAR* psFilename)
{
...
void GrassShader::initShader(WCHAR* vsFilename, WCHAR* gsFilename, WCHAR* psFilename)
{
...
XMFLOAT4 floatThree_to_floatFour(XMFLOAT3 input, float w_val = 0.f) //converts a float3 to a float4 for padding
{
	return XMFLOAT4(input.x, input.y, input.z, w_val);
}

void GrassShader::setShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX &worldMatrix, const XMMATRIX &viewMatrix, const XMMATRIX &projectionMatrix, ID3D11ShaderResourceView* texture, Light* light, XMFLOAT3 _camPos, float time)
{
	...
	lightPtr->position = floatThree_to_floatFour(light->getPosition());
	...
	camPtr->camPos = floatThree_to_floatFour(_camPos);
	camPtr->time = time;
...
void GrassShader::render(ID3D11DeviceContext* deviceContext, int indexCount)
{
