...
void main(triangle InputType input[3], inout TriangleStream<OutputType> triStream)
{
	...
	output.worldPosition = mul(input[1].position + float4(0.0f, 1.0f, 0.0f, 0.0f), worldMatrix).xyz;
	output.position = mul(input[1].position + float4(0.0f, 1.0f, 0.0f, 0.0f), worldMatrix);
	output.position = mul(output.position, viewMatrix);
	output.position = mul(output.position, projectionMatrix);
	output.tex = input[1].tex;
	output.normal = input[1].normal;
