...
XMFLOAT4 float3_to_float4(XMFLOAT3 in) {
	return XMFLOAT4(in.x, in.y, in.z, 0.f);
}

void LightShader::setShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX &worldMatrix, const XMMATRIX &viewMatrix, const XMMATRIX &projectionMatrix, ID3D11ShaderResourceView* texture, Light *light, Light *light_dir, XMFLOAT4 wave_info)
{
	HRESULT result;
	...
	lightPtr->ambient[1] = light_dir->getAmbientColour();
	lightPtr->diffuse[1] = light_dir->getDiffuseColour();
	lightPtr->direction = float3_to_float4(light_dir->getDirection());
